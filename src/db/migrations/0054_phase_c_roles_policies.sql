...
alter table "tenants" drop constraint if exists "tenants_owner_user_id_users_id_fk";--> statement-breakpoint
...
alter table "users" drop constraint if exists "users_partner_link_matches_role";--> statement-breakpoint
...
drop policy if exists role_capabilities_scope on role_capabilities;--> statement-breakpoint
create policy role_capabilities_scope on role_capabilities for all
  using ((tenant_id = app_current_tenant()) and (app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])))
  with check ((tenant_id = app_current_tenant()) and (app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])));--> statement-breakpoint

-- ── (3c) team_invites hardening (audit-tenancy F-12): staff-role CHECK (TEXT literals,
-- no enum coupling), FK-covering index, and one-live-invite-per-address race safety.
-- Deliberately NOT mirrored into schema.ts index declarations: this file is SQL-only
-- (no snapshot), and declaring them in schema.ts would make the next drizzle-kit
-- generate re-emit them against the 0053 baseline.
alter table "team_invites" drop constraint if exists "team_invites_role_is_staff";--> statement-breakpoint
alter table "team_invites" add constraint "team_invites_role_is_staff"
  check ("role" in ('admin', 'member', 'viewer'));--> statement-breakpoint
create index if not exists "team_invites_invited_by_idx" on "team_invites" ("invited_by_user_id");--> statement-breakpoint
create unique index if not exists "team_invites_live_email_idx" on "team_invites" ("tenant_id", lower("email"))
...
create policy users_scope on users for all
  using (((tenant_id = app_current_tenant()) AND ((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) OR (id = app_current_user()))))
...
create policy partners_scope on partners for all
  using (((tenant_id = app_current_tenant()) AND ((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) OR (id = app_current_partner()))))
  with check ((tenant_id = app_current_tenant()));--> statement-breakpoint
...
create policy audit_log_scope on audit_log for all
  using (((tenant_id = app_current_tenant()) AND ((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) OR (actor_user_id = app_current_user()))))
  with check ((tenant_id = app_current_tenant()));--> statement-breakpoint
drop policy if exists leads_scope on leads;--> statement-breakpoint
create policy leads_scope on leads for all
  using (((tenant_id = app_current_tenant()) AND ((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) OR (COALESCE(manual_partner_id, partner_id) = app_current_partner()))))
  with check (((tenant_id = app_current_tenant()) AND ((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) OR ((app_current_role() = 'partner'::text) AND (COALESCE(manual_partner_id, partner_id) = app_current_partner()) AND (deleted_at IS NULL)))));--> statement-breakpoint
...
create policy lead_notes_scope on lead_notes for all
  using (((tenant_id = app_current_tenant()) AND (((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) AND (author_role = 'admin'::author_role)) OR ((app_current_role() = 'partner'::text) AND (author_role = 'partner'::author_role) AND (lead_id IN ( SELECT leads.id
   FROM leads
  ...
  with check (((tenant_id = app_current_tenant()) AND (author_user_id = app_current_user()) AND (((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) AND (author_role = 'admin'::author_role) AND (lead_id IN ( SELECT leads.id
   FROM leads
...
create policy lead_status_history_scope on lead_status_history for all
  using (((tenant_id = app_current_tenant()) AND ((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) OR ((lead_id IN ( SELECT leads.id
   FROM leads
  ...
  with check (((tenant_id = app_current_tenant()) AND (((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) AND (lead_id IN ( SELECT leads.id
   FROM leads
...
create policy lead_tasks_scope on lead_tasks for all
  ...
  with check (((tenant_id = app_current_tenant()) AND (author_user_id = app_current_user()) AND ((assigned_to_user_id IS NULL) OR (assigned_to_user_id IN ( SELECT users.id
   FROM users
  WHERE (users.tenant_id = app_current_tenant())))) AND (((app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) AND (author_role = 'admin'::author_role) AND (lead_id IN ( SELECT leads.id
   FROM leads
  WHERE (leads.tenant_id = app_current_tenant())))) OR ((app_current_role() = 'partner'::text) AND (author_role = 'partner'::author_role) AND (lead_id IN ( SELECT leads.id
   FROM leads
  WHERE ((leads.tenant_id = app_current_tenant()) AND (COALESCE(leads.manual_partner_id, leads.partner_id) = app_current_partner()) AND (leads.deleted_at IS NULL) AND (leads.created_at < (now() - '00:05:00'::interval)))))))));--> statement-breakpoint
...
create policy lead_tags_scope on lead_tags for all
  using (((tenant_id = app_current_tenant()) AND (app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text]))))
  with check (((tenant_id = app_current_tenant()) AND (app_current_role() = ANY (ARRAY['admin'::text, 'member'::text, 'viewer'::text])) AND (added_by_user_id = app_current_user()) AND (lead_id IN ( SELECT leads.id
   FROM leads
