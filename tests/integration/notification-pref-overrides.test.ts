...
suite("WP-NF2 PR A: per-subject prefs + tokenized unsubscribe", () => {
  ...
  beforeAll(async () => {
    ...
    await db.insert(schema.users).values([
      { id: id.admin, tenantId: a.id, email: "admin@nf2.test", role: "admin" },
      { id: id.admin2, tenantId: a.id, email: "admin2@nf2.test", role: "admin" },
      { id: id.seat, tenantId: a.id, email: "seat@nf2.test", role: "partner", partnerId: p.id },
      { id: id.seat2, tenantId: a.id, email: "seat2@nf2.test", role: "partner", partnerId: p.id },
      { id: id.strangerAdmin, tenantId: b.id, email: "admin@nf2.test", role: "admin" },
    ]);
  ...
  it("NTF-13: ensureSubjectToken get-or-creates ONE row per subject and returns a stable token", async () => {
    ...
    expect(tokenId).toHaveLength(22); // 16 bytes, base64url unpadded
