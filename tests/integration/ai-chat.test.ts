...
suite("WP-AI-1 Task 11: assistant core — gate + streamText (AIA-01..06)", () => {
  ...
  describe("assistantGate: the four refusal branches", () => {
    it("gate: disabled tenant → ai_disabled", async () => {
      ...
      const res = await assistantGate(db, scope, { appEnv: "development", aiTier: "paid", hasGatewayKey: true, now: NOW });
    ...
    it("gate: production + free-dev tier → ai_disabled (503)", async () => {
      const res = await assistantGate(db, scopeA, { appEnv: "production", aiTier: "free-dev", hasGatewayKey: true, now: NOW });
