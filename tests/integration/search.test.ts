...
import { adminScope, scopeContextMock, setRouteScope } from "./_route-harness";
...
suite("SRCH: globalSearch (SRCH-01)", () => {
  ...
  it("SRCH-01: partners match on name/ref/email; revoked and deleted partners are excluded", async () => {
    ...
    expect((await globalSearch(scopeA(), "ops@cedarridge.test")).partners.rows.map((p) => p.id)).toEqual([ids.partnerA]);
...
suite("SRCH: GET /api/search — admin gate + envelope (SRCH-01/04)", () => {
  ...
  async function cleanup() {
    const t = await db.select({ id: schema.tenants.id }).from(schema.tenants).where(inArray(schema.tenants.slug, [SLUG_R]));
    const tids = t.map((x) => x.id);
    if (tids.length === 0) return;
    await db.delete(schema.leads).where(inArray(schema.leads.tenantId, tids));
    await db.delete(schema.uploads).where(inArray(schema.uploads.tenantId, tids));
