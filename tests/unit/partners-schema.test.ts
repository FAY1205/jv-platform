...
describe("PartnerCreateSchema", () => {
  it("PARTNERS-VAL-01: requires a non-empty name and trims it", () => {
    expect(PartnerCreateSchema.safeParse({ name: "   " }).success).toBe(false);
    const ok = PartnerCreateSchema.parse({ name: "  Acme Capital  " });
    expect(ok.name).toBe("Acme Capital");
  });

  it("PARTNERS-VAL-02: accepts a minimal partner (name only)", () => {
    const ok = PartnerCreateSchema.parse({ name: "Solo" });
    expect(ok.email).toBeUndefined();
  });

  it("PARTNERS-VAL-03: rejects a malformed email but allows a blank one", () => {
    expect(PartnerCreateSchema.safeParse({ name: "X", email: "not-an-email" }).success).toBe(false);
    expect(PartnerCreateSchema.parse({ name: "X", email: "" }).email).toBeUndefined();
    expect(PartnerCreateSchema.parse({ name: "X", email: "a@b.co" }).email).toBe("a@b.co");
  });
});

describe("PartnerUpdateSchema", () => {
  it("PARTNERS-VAL-04: allows partial updates and never accepts color/status", () => {
    const ok = PartnerUpdateSchema.parse({ dealTerms: "50/50" });
