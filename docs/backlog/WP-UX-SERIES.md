...
| WP-UX-4 | Map & chart honesty (legend/hatch/sentiment) | T4 | High | waived | ✅ 2026-08-16 · dark brand/warn chart-hue separation **DONE 2026-08-18 (PR #106, →`--info`)** · on-map choropleth labels + anchored min/max legend **DONE 2026-08-18 (PR #127, ADR-0050)** — opaque `--surface` chips ("NE · 7") on every gap state via the committed anchor table `src/lib/geo/us-state-anchors.ts`; `HeatLegend` anchored to the real data range. **Slice fully closed.** |
...
| WP-UX-7 | Empty-state, copy & dialog polish | T5/T6 | High | no | ✅ 2026-08-17 · **deferred cuts DONE 2026-08-18 (PR #107)**: scoring-card range/badge fixed-width slots, "Required: Yes" dedup (annotate only the Mortgage exception), Tags create-row colour picker (+ Auto default), portal "My Tasks" title de-dup (`MyTasksList title` prop) |
| WP-UX-8 | Shared paper cuts + dark parity | T6 | Medium | no | ✅ 2026-08-17 · **dark-mode pass DONE 2026-08-18 (PR #106)**: map stroke theme-parity + non-territory land raised (new `--map-line`/`--map-land`/`--map-land-line` pairs, zero light change), portal `--border-soft` hairline lift, dark donut brand/warn collapse → `--info`. Portal-dark re-capture **DONE 2026-08-18** (real `DEV_SCOPE_ROLE=partner` shim session + a capture guard that fails on admin redirects — portal dark verified sound, no token change needed). STILL DEFERRED (owner eyeball): dark rim-light shadows `--sh-sm/md`, `--chart-cat-*` ADR |
