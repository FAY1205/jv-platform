...
| N4 | Search v2 (Postgres search engine: partial words, ranked, phone-proof — Ctrl-K + leads + portal) | ✅ **DONE (PR #146, merged + PROD-VERIFIED 2026-08-19).** SRCH-06 multi-term AND (max 6, DM-12) · SRCH-07 phone-digit + escaped patterns on all three surfaces (leads-list/portal patterns were UNESCAPED — fixed; activity's sibling fixed in #145) · SRCH-08 trigram-ranked Ctrl-K (word_similarity arg-order mutation-tested; deterministic refId tie-break) · migration 0056 (pg_trgm + 5 GIN indexes) + ADR-0051 (supersedes SRCH-03's no-extension clause). Prod verified live: pg_trgm 1.6 in public, 5/5 indexes, ledger 57/57 `when`=1787136871489, word_similarity resolves unqualified. Reviews: pr-reviewer + audit-tenancy PASS×5 + audit-data PASS×5, all findings applied (incl. the fail-closed empty-match guard the PR body had wrongly claimed). Follow-up #147: main's e2e smoke (PR runs skip e2e) had been red since #142's AuthCardHeader — fixed, main fully green. | ✅ owner greenlit ("migrate when reviewed, tested, confident") — condition met |
| NF2 | Notifications v2 (new types, /notifications page, per-user + partner settings, unsubscribe) | ✅ **DONE (PRs #150 + #151 + #152, all merged 2026-08-20; migration 0057 PROD-VERIFIED).** #150: per-SUBJECT prefs overlay (`notification_pref_overrides` — user seats AND partner orgs; tenant matrix stays the default layer, overlay wins field-wise, `allEmailsOff` kill switch never touches in-app), tokenized `/unsubscribe` (split token, timingSafeEqual + dummy-compare, AUT-05 uniform envelope, `UNSUBSCRIBE_THROTTLE` + `withUniformTiming`, csrf-conformance allowlist, Referrer-Policy no-referrer), `/api/me/notification-prefs`, per-recipient footer links in every notification email, ADR-0052 (cascade). #151: `/notifications` for BOTH roles (shared component, keyset cursor with MICROSECOND precision + id tie-break, ownerWhere mutation-verified), Preferences card (the partner-facing surface; member/viewer get it too), bell "View all". #152: 4 new types — `task_assigned` (both buckets; C-82a closed) · `partner_note` (PRN-13-safe direction, note body NEVER in any payload) · `import_result` (ING-08 loud-failure pairing; success skips the actor) · `partner_activated` (invited→active one-shot, tos/accept gained the missing tenant pin — audit-tenancy HIGH, prod probe clean); all email-default-OFF. Reviews: pr-reviewer ×3 + audit-security + audit-tenancy ×3 + audit-data — 0 unresolved findings (fix rounds 17/18 + 6/6 + 9/9 applied). De-flaking PR A's suite exposed + fixed a real frozen-catalog regression. Candidates C-101–C-114. Deferred-for-owner in Slice 8. |
| NF2b | Notification prefs simplification (owner decision 2026-08-20: **no workspace layer — every user controls their own**) | ⏳ **PR OPEN.** Resolution is now `DEFAULT_NOTIFICATION_PREFS ⊕ the subject's own overlay`, full stop: the tenant `settings.notification_prefs` layer is removed from every read path (outbox / events / task-reminders / run-upload), `loadNotificationPrefs`/`saveNotificationPrefs`/`mergeNotificationPrefs`/`NotificationPrefsSchema` are deleted as dead code, and `GET/PUT /api/settings/notifications` is retired. **NO migration** — prod had zero `notification_prefs` rows, so live behaviour is unchanged; the key simply stops being read. Settings → Notifications becomes the PERSONAL page (the same card the portal mounts — one editor, two mounts — reachable by member/viewer with **no gating change needed**: the hub gates on stream, not tier); /notifications links there for the admin stream and keeps the inline card in the portal. Org-addressed partner email is now defaults ⊕ the partner-ORG unsubscribe overlay (documented at the emit site). Task-reminder tenant early-out removed — **deferred item 8's asymmetry dissolves**. C-108 closed as a deliberate exemption; the unsubscribe page links onward to /notifications. Details: `docs/backlog/WP-NF2.md` §11. | — |
...
| **NF2 closeout 2026-08-20 — ALL THREE PRs MERGED (#150 foundation · #151 pages · #152 new types); deferred-for-owner (safe reversible defaults applied):** (1) **Email legs of all four new types default OFF** (in-app ON) — the assigned_lead precedent; each flip is one line. (2) **`import_result` success skips the acting admin** (run_summary already covers them); failures notify everyone INCLUDING the actor, and **repeated failed attempts each notify** (loud per ING-08). ✅ **ANSWERED 2026-08-20 (WP-NF2b): the per-user mute IS the control — NO repeat cap.** An admin drowning in a re-uploaded broken file turns `import_result` off for themselves; the loudness stays for everyone who wants it. The `partner_note` coalescing half of C-104 stays open. (3) **Ops types go to the admin TIER only** (partner_note / import_result / partner_activated — the Phase C F-8 default); say if member seats should see partner notes. (4) **Env `adminAllowlist` extra addresses get no unsubscribe link** and stay tenant-level-gated (they're your ops config, not seats). (5) **Unsubscribe tokens are stored server-side with a plaintext secret half** (capability = email-reduction only; comparison constant-time; no new env var) — swap to HMAC+env later invalidates old links (C-102 adds rotation). (6) ✅ **ANSWERED 2026-08-20 (WP-NF2b): MERGED INTO SETTINGS as the personal page.** `/settings/notifications` no longer edits a workspace matrix — it IS "my notifications", mounting the same card the portal renders, reachable by every admin-stream seat (member/viewer included — no gating change was needed). The /notifications page links there for the admin stream; the PORTAL keeps its inline card, because a partner cannot enter admin Settings. ONE editor component, two mounts. (7) **"Pause all notification emails" never touches transactional/auth email** (NTF-05) — stated in the UI copy. (8) ✅ **ANSWERED 2026-08-20 (WP-NF2b): the asymmetry is DISSOLVED with the tenant layer.** There is no workspace mute left to early-out on, so the reminder sweep resolves every recipient on its own overlay like every other emit. Cost: a tenant whose every seat muted `task_due` now pays an indexed, limited due select per tick instead of one settings read (overlay + token loads are still one query each). (9) ✅ **ANSWERED 2026-08-20 (WP-NF2b): the ToS-gate exemption is DELIBERATE** (C-108 closed). `GET /api/notifications` stays un-gated — the LGL-01 acceptance boundary is page-level and the feed carries lead refs + status words + filenames + fixed sentences, never seller PII (NTF-16). Recorded in the route header with the condition that would kill it (a feed that ever carries lead CONTENT). (10) **`/api/unsubscribe` is deliberately CSRF-exempt and Origin-free** (tokenized, sessionless; executable allowlist records it) — required for future RFC 8058 one-click (C-103); "hardening" it with Origin checks is an ADR-gated breaking change. (11) **C-101 is a pre-existing HIGH worth prioritizing**: password-reset/signup emails build links from the request Host header (account-takeover shape if the platform doesn't pin Host) — NF2's own links used env.APP_URL; the sweep is small. (12) **Owner hands-on pass** of /notifications + the prefs card + an unsubscribe click still pending (standing Slice 8 row). ✅ **2026-08-20 (WP-NF2b): the unsubscribe page now LINKS ONWARD** — a quiet "Manage all notification preferences" footer link to /notifications, so the one-click off switch is no longer a dead end for someone who wanted "less", not "none". Signed-out lands on /login?next=/notifications and returns. Known gap (candidate): the proxy picks the login SCREEN by path prefix, so a signed-out PARTNER lands on the admin password screen; their in-mail control is unaffected. |
| ✅ **NF2 kickoff decisions 2026-08-19 (owner answered):** (1) **New types — ALL FOUR**: task assigned/reassigned to you (C-82a) · partner-added-note → admins (PRN-13-safe direction, note body never in the payload) · import completed/failed (pairs with ING-08 loud-failure) · partner accepted invite/ToS. **Owner qualifier: every type must be CONFIGURABLE** — per-user in-app + email toggles in the existing prefs matrix; new types are rows there, never hardwired. (2) **Unsubscribe = per-type one-click links in every email footer + a global all-notification-email kill switch** (maps onto the prefs table). (3) **/notifications full page for BOTH roles** (admin + portal, same scoped feed endpoint). (4) **Q8 re-affirmed: keep deferring** — the mobile-assistant modality lands with the ai-redesign-spec as one piece; C-62 stays parked; NF2 does not touch the assistant. Next re-ask: AI-redesign greenlight. | ✅ |
