...
| **⚠️ PR #132 — WP-NF1 D1 bell-read index (migration 0055): GREENLIGHT TO MERGE?** Built + pr-reviewed (comment fix applied) + CI-green + applied cleanly to the test DB; journal `when` verified above the trap. Tier A: migrate-on-merge auto-applies to prod. Adds `(tenant_id,user_id,created_at desc)` + partial unread index, drops the superseded `notifications_user_idx` (FK-cover note in the PR body: users hard-delete only pre-activation, which cannot have produced notification rows). | ⏳ |
