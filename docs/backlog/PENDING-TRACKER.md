...
| **NF2 closeout 2026-08-20 — ALL THREE PRs MERGED (#150 foundation · #151 pages · #152 new types); deferred-for-owner (safe reversible defaults applied):** (1) **Email legs of all four new types default OFF** (in-app ON) — the assigned_lead precedent; each flip is one line. (2) **`import_result` success skips the acting admin** (run_summary already covers them); failures notify everyone INCLUDING the actor, and **repeated failed attempts each notify** (loud per ING-08) — say if you want a repeat-cap (C-104). (3) **Ops types go to the admin TIER only** (partner_note / import_result / partner_activated — the Phase C F-8 default); say if member seats should see partner notes. (4) **Env `adminAllowlist` extra addresses get no unsubscribe link** and stay tenant-level-gated (they're your ops config, not seats). (5) **Unsubscribe tokens are stored server-side with a plaintext secret half** (capability = email-reduction only; comparison constant-time; no new env var) — swap to HMAC+env later invalidates old links (C-102 adds rotation). (6) **The per-user prefs surface lives ON the /notifications page** (both roles) rather than a settings tab — relocatable; the admin tenant-defaults matrix is unchanged apart from the new rows. (7) **"Pause all notification emails" never touches transactional/auth email** (NTF-05) — stated in the UI copy. (8) **A fully-muted tenant default is the ONE ceiling a user overlay cannot widen on task reminders** (cron cost early-out; overlays can widen everything else — deliberate asymmetry, flagged in code). (9) **ToS-gate on the notification feed is an open owner call** (C-108): a re-accept-gated seat can still read its bell/page today (lead refs + status words only, never seller PII). (10) **`/api/unsubscribe` is deliberately CSRF-exempt and Origin-free** (tokenized, sessionless; executable allowlist records it) — required for future RFC 8058 one-click (C-103); "hardening" it with Origin checks is an ADR-gated breaking change. (11) **C-101 is a pre-existing HIGH worth prioritizing**: password-reset/signup emails build links from the request Host header (account-takeover shape if the platform doesn't pin Host) — NF2's own links used env.APP_URL; the sweep is small. (12) **Owner hands-on pass** of /notifications + the prefs card + an unsubscribe click still pending (standing Slice 8 row). |
