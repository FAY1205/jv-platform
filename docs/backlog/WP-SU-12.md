...
**Correct reference already in the codebase:** `src/app/api/auth/login/route.ts:65` records `store.record(email, ip, KIND, success === true)` — a wrong password → `false` (feeds lockout, intended); a success → `true` (does not). The three endpoints simply don't follow this pattern.

**Accuracy note (per audit-finding-accuracy discipline):** the originating writeup described a `settle(attemptId, …)` / "WP-SU-9 reservation" two-phase pattern. **No such pattern exists** — the real code is a one-shot `attempts.record(...)`. The vulnerability is real and byte-for-byte pre-existing (confirmed via `git show`); the fix is *simpler* than the writeup implies. WP-SU-9 did not introduce or worsen this.
...
| Endpoint | Today | Fix |
|---|---|---|
| otp/request | `record(…, false)` always | `record(…, true)` always — a code *request* is not a credential check |
| reset/request | `record(…, false)` always | `record(…, true)` always — same |
| otp/verify | `record(…, false)` before the challenge check | record the **actual outcome**: `false` **only** when an active challenge exists and the code is genuinely wrong; `true` for success / no-challenge / expired / too-many |
