...
| DM-13 | **Concurrent, out-of-transaction structural changes (ADR-0048).** Any index or column add on a *populated prod* table uses the lock-light form (`CREATE INDEX CONCURRENTLY`, or `ADD COLUMN` with no default rewrite) and is applied **out-of-transaction, manually to prod** — never inside a `drizzle-kit migrate` single-file transaction (a plain `CREATE INDEX` takes a write-blocking `ShareLock`, forbidden post-launch; `CONCURRENTLY` cannot run in a txn). Split a mixed change into two files (transactional DDL + a manual concurrent step). Such changes, plus RLS/`GRANT`/`REVOKE`/index-rename migrations, are hand-authored SQL that bypasses `drizzle-kit generate` and therefore ships **without** a `meta/*_snapshot.json` — the snapshot chain bridges them by design, `drizzle-kit check` validates it, and live introspection (`pg_indexes`/`pg_policies`) is the source of truth for anything applied out-of-band. |
...
| SEC-09 | **Availability posture:** a transient auth-backend/infrastructure outage (a thrown fault, NOT a credential result) returns **503 + `Retry-After`** — a retryable status, not a hard 500 or a 401 masquerade. It is uniform-timed (behind the AUT-05 floor), account-independent (never an existence oracle), logged with a correlated traceId, and never feeds the AUT-04 lockout ladder. (SEC-08 is reserved for the shipped security-headers requirement tracked in ENGINEERING_STANDARDS; not yet mirrored here.) |
